...
with sync_playwright() as pw:
    ...
    # Reproduce the actual post-upload DOM state: imported Excel result + 12 photo filenames.
    p.evaluate(r'''() => {
      ...
      if(result && summary){
        result.hidden=false;
        const rows=[
          ['船積港','横浜港'],['陸揚港','シンガポール港'],['貨物質量','8.559 t（G/W）'],
          ['貨物名','TEST CARGO / LONG DESCRIPTION FOR PRINT LAYOUT VERIFICATION'],
          ['個数','48個'],['コンテナ番号','SKDG1234567'],['申請番号','2026-012345'],
          ['対象危険物','UN1203 / UN1993 / UN3082']
        ];
        summary.innerHTML=rows.map(([a,b])=>`<div class="import-summary-item"><strong>${a}</strong><span>${b}</span></div>`).join('');
      }
      const status=document.getElementById('ctuExcelStatus');
      if(status) status.textContent='sample_application.xlsx を読み込みました。';
      const q=document.getElementById('part542PhotoQueue');
      if(q){
        const ul=document.createElement('ul'); ul.className='v1382-photo-queue-list';
        for(let i=1;i<=12;i++){
          const li=document.createElement('li'), btn=document.createElement('button');
          btn.type='button'; btn.className='v1382-photo-queue-btn';
          btn.textContent=`${i}. 現場写真_${String(i).padStart(2,'0')}_固縛確認用.jpg`;
          li.appendChild(btn); ul.appendChild(li);
        }
        q.replaceChildren(ul);
      }
      const ps=document.getElementById('photoStatus'); if(ps) ps.textContent='12枚の写真を登録済みです。';
    }''')
    if p.evaluate("typeof window.SKDG_CTU_PRINT_EXPANDED_SYNC_V13102 === 'function'"):
        p.evaluate("window.SKDG_CTU_PRINT_EXPANDED_SYNC_V13102()")
    p.wait_for_timeout(200)

    out=ROOT/'docs'/'verification'/'v1.3.102-expanded'; out.mkdir(parents=True,exist_ok=True)
    # Expanded screen evidence while two-column mode is still active.
    screen_audit=p.evaluate(r'''() => {
      const r=s=>{const e=document.querySelector(s); if(!e)return null; const x=e.getBoundingClientRect();return {x:x.x,y:x.y,w:x.width,h:x.height,right:x.right,bottom:x.bottom}};
      return {step1:r('#ctuExcelRoutePanel'),step2:r('#photoInputPanel'),step3:r('[data-ctu-step="3"]'),step4:r('[data-ctu-step="4"]'),overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth,appClass:document.body.classList.contains('ctu-print-application-expanded'),photoClass:document.body.classList.contains('ctu-print-photo-has-files'),photoCount:document.body.dataset.ctuPrintPhotoCount};
    }''')
    p.locator('#ctuExcelRoutePanel').scroll_into_view_if_needed(); p.wait_for_timeout(100); p.screenshot(path=str(out/'v13102_expanded_screen_step1_2.png'),full_page=False)
    p.screenshot(path=str(out/'v13102_expanded_screen_fullpage.png'),full_page=True)
    ...
    audit=p.evaluate("""() => {
      ...
      return {
        ...
        supportDisplay:disp('#ctuSupportSecuringPanel'), bracingDisplay:disp('#ctuBracingAssist'), deficiencyDisplay:disp('#deficiencySupportPanel'), photoQueueDisplay:disp('#part542PhotoQueue'),
    ...
    pdf_path=out/'v13102_expanded_application_photos_A4.pdf'
...
ck('expanded print keeps Step 3 new-page grouping',audit['step3']['breakBefore']=='page')
...
ck('expanded A4 report remains six pages',page_count==6)
ck('page 1 contains imported application summary', '申請書から取得した情報' in page_texts[0] and '2026-012345' in page_texts[0])
ck('page 1 contains photo step with imported Step 1', '写真を撮影' in page_texts[0])
ck('page 2 contains Steps 3 and 4', '輸送条件と貨物を確認' in page_texts[1] and 'CTU' in page_texts[1] and '参考摩擦係数' in page_texts[1])
ck('expanded state has application-expanded class','ctu-print-application-expanded' in audit['classes'])
ck('expanded state has photo-files class','ctu-print-photo-has-files' in audit['classes'])
ck('expanded screen has no horizontal overflow',not screen_audit['overflow'])
ck('expanded two-column Step 1 and 2 remain side by side',screen_audit['step1']['x'] < screen_audit['step2']['x'] and abs(screen_audit['step1']['y']-screen_audit['step2']['y']) < 2)
ck('expanded Step 3 begins below taller Step 1/2 row',screen_audit['step3']['y'] >= max(screen_audit['step1']['bottom'],screen_audit['step2']['bottom'])-2)
ck('expanded photo count is 12',screen_audit['photoCount']=='12')
ck('photo filename queue is removed from print flow',audit['photoQueueDisplay']=='none')
...
(out/'v13102_expanded_print_verification.json').write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8')
with (out/'v13102_expanded_print_verify.log').open('w',encoding='utf-8') as f:
  for n,v in checks: f.write(('PASS ' if v else 'FAIL ')+n+'\n')
  f.write(f'TOTAL {len(checks)} / PASS {result["passCount"]} / FAIL {result["failCount"]}\n')
print((out/'v13102_expanded_print_verify.log').read_text(encoding='utf-8'))
