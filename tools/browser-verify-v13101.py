...
OUT=ROOT/'docs'/'verification'/'v1.3.101'
...
ESSENTIAL=(
 ...
 'v1398-ctu-usability.js','v13100-ctu-ui-consistency.js','v13101-ctu-print-state.js'
)
script_parts=[]
...
(OUT/'v13101_harness_sources.json').write_text(json.dumps({
...
with sync_playwright() as pw:
    ...
    p.screenshot(path=str(OUT/'v13101_pc_one_column_top.png'),full_page=False)
    ...
    p.screenshot(path=str(OUT/'v13101_pc_manual_two_column_top.png'),full_page=False)
    ...
    p.locator('.ctu-numbered-step-card[data-ctu-step="5"]').scroll_into_view_if_needed(); p.wait_for_timeout(100); p.screenshot(path=str(OUT/'v13101_step5_one_column.png'),full_page=False)
    p.locator('.ctu-numbered-step-card[data-ctu-step="6"]').scroll_into_view_if_needed(); p.wait_for_timeout(100); p.screenshot(path=str(OUT/'v13101_step6_wall.png'),full_page=False)
    p.locator('.ctu-numbered-step-card[data-ctu-step="7"]').scroll_into_view_if_needed(); p.wait_for_timeout(100); p.screenshot(path=str(OUT/'v13101_step7_result.png'),full_page=False)
    b.close()
    ...
    p.screenshot(path=str(OUT/'v13101_wide_auto_two_column.png'),full_page=False); b.close()
    ...
    p.screenshot(path=str(OUT/'v13101_mobile_top.png'),full_page=False); b.close()
...
(OUT/'v13101_browser_verification.json').write_text(json.dumps(results,ensure_ascii=False,indent=2),encoding='utf-8')
with (OUT/'v13101_browser_verify.log').open('w',encoding='utf-8') as f:
  for n,v in checks: f.write(('PASS ' if v else 'FAIL ')+n+'\n')
  f.write(f'TOTAL {len(checks)} / PASS {results["passCount"]} / FAIL {results["failCount"]}\n')
print((OUT/'v13101_browser_verify.log').read_text(encoding='utf-8'))
