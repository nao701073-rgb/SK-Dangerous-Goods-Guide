...
with sync_playwright() as pw:
    ...
    p.screenshot(path=str(OUT/'v13103_material_support_two_column_actual.png'),full_page=False)
    ...
    pdf=OUT/'v13103_material_support_A4.pdf'
...
(OUT/'v13103_material_support_print.json').write_text(json.dumps(res,ensure_ascii=False,indent=2),encoding='utf-8')
log='\n'.join(('PASS ' if x['pass'] else 'FAIL ')+x['name'] for x in checks)+f"\nTOTAL {len(checks)} / PASS {res['passCount']} / FAIL {res['failCount']}\n"
(OUT/'v13103_material_support_print.log').write_text(log,encoding='utf-8')
print(log)
