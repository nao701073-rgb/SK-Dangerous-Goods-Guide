...
const currentRelease=String(version.version||'');
const currentPart=Number(version.part||0);
const currentBuildTag=`part${currentPart}`;
...
check('申請番号管理の現行版識別',appsHtml.includes(`content="${currentBuildTag}"`),currentBuildTag);
...
check('固縛力画面の現行版識別',ctuHtml.includes(`content="${currentBuildTag}"`),currentBuildTag);
...
check('VERSION現行版整合',currentPart>=529&&currentRelease===currentBuildTag,JSON.stringify(version));
...
const report={generatedAt:new Date().toISOString(),release:currentRelease,status:failed.length?'failed':'passed',total:checks.length,passed:checks.length-failed.length,failed:failed.length,checks};
const out=path.join(root,'docs','part529_共通案件情報ポリシー・静的検証レポート.json');
