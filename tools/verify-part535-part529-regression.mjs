...
check('申請番号管理の現行版識別',appsHtml.includes('content="part535"'));
...
check('固縛力画面の現行版識別',ctuHtml.includes('content="part535"'));
...
check('VERSION current',version.version==='part535'&&version.part===535);
...
const report={generatedAt:new Date().toISOString(),release:'part535-part529-regression',status:failed.length?'failed':'passed',total:checks.length,passed:checks.length-failed.length,failed:failed.length,checks};
const out=path.join(root,'docs','part535_Part529共通案件情報回帰検証レポート.json');
