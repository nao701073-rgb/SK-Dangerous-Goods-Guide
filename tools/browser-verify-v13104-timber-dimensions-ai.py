...
import json,re,sys
ROOT=Path(sys.argv[1] if len(sys.argv)>1 else '.').resolve();PAGE=ROOT/'pages'/'ctu-securing-calculator.html';OUT=ROOT/'docs'/'verification'/'v1.3.104';OUT.mkdir(parents=True,exist_ok=True)
...
ESSENTIAL=(
 ...
 'v13100-ctu-ui-consistency.js','v13101-ctu-print-state.js','v13102-ctu-expanded-print-state.js','v141-photo-ai-container-fit.js','v13103-ctu-actionable-confirmation-guide.js','v13103-ctu-material-strength-linkage.js','v13104-ctu-timber-dimensions-ai.js'
)
scripts=[]
...
with sync_playwright() as pw:
    ...
    p.select_option('#quickSupportMaterial','frp');p.wait_for_timeout(120)
    ...
    p.locator('#quickTimberUploadAiPanel').scroll_into_view_if_needed();p.wait_for_timeout(100);p.screenshot(path=str(OUT/'v13104_initial_upload_ai_step2.png'),full_page=False)
    p.click('#quickTimberUploadAiJump');p.wait_for_timeout(520)
    ck('jump from Step2 points to support material before timber selection',p.evaluate("document.activeElement?.id==='quickSupportMaterial'"))
    p.select_option('#quickSupportMaterial','timber');p.wait_for_timeout(180);p.locator('#quickTimberThicknessW').scroll_into_view_if_needed();p.wait_for_timeout(100);p.screenshot(path=str(OUT/'v13104_initial_upload_ai_step5_staged.png'),full_page=False)
    ...
    p.set_viewport_size({'width':2600,'height':1200});p.wait_for_timeout(280);p.locator('.ctu-numbered-step-card[data-ctu-step="5"]').scroll_into_view_if_needed();p.wait_for_timeout(120);p.screenshot(path=str(OUT/'v13104_timber_whl_ai_step5.png'),full_page=False)
...
(OUT/'v13104_timber_dimensions_ai_browser.json').write_text(json.dumps(res,ensure_ascii=False,indent=2),encoding='utf-8')
log='\n'.join(('PASS ' if x['pass'] else 'FAIL ')+x['name'] for x in res['checks'])+f"\nTOTAL {len(res['checks'])} / PASS {res['passCount']} / FAIL {res['failCount']}\nERRORS {len(res['errors'])}\n";(OUT/'v13104_timber_dimensions_ai_browser.log').write_text(log,encoding='utf-8');print(log)
