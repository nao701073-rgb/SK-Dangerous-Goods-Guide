...
check('申請番号管理の現行版識別',appsHtml.includes('content="part530"'));
...
check('固縛力画面の現行版識別',ctuHtml.includes('content="part530"'));
...
check('VERSION part530',version.version==='part530'&&version.part===530);
...
const report={generatedAt:new Date().toISOString(),release:'part530',status:failed.length?'failed':'passed',total:checks.length,passed:checks.length-failed.length,failed:failed.length,checks};
const out=path.join(root,'docs','part530_Part529共通案件情報回帰検証レポート.json');
