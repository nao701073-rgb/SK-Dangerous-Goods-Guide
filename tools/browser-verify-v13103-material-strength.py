...
OUT=ROOT/'docs'/'verification'/'v1.3.103'
...
with sync_playwright() as pw:
    ...
    # support timber
    choose(p,'#quickSupportMaterial','timber');choose(p,'#quickSupportProfile','timber-batten-50x100-l22')
    ...
    choose(p2,'#quickSupportMaterial','timber');choose(p2,'#quickSupportProfile','timber-batten-75x100-l22')
...
(OUT/'v13103_material_strength_browser.json').write_text(json.dumps(res,ensure_ascii=False,indent=2),encoding='utf-8')
log='\n'.join(('PASS ' if x['pass'] else 'FAIL ')+x['name'] for x in checks)+f"\nTOTAL {len(checks)} / PASS {res['passCount']} / FAIL {res['failCount']}\nERRORS {len(res['errors'])}\n"
(OUT/'v13103_material_strength_browser.log').write_text(log,encoding='utf-8')
print(log)
