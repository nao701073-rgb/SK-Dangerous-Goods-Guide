...
with sync_playwright() as pw:
    ...
    out=ROOT/'docs'/'verification'/'v1.3.101'; out.mkdir(parents=True,exist_ok=True)
    ...
    pdf_path=out/'v13101_actual_A4_print.pdf'
...
ck('A4 report is six pages',page_count==6)
...
(out/'v13101_print_verification.json').write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8')
with (out/'v13101_print_verify.log').open('w',encoding='utf-8') as f:
  for n,v in checks: f.write(('PASS ' if v else 'FAIL ')+n+'\n')
  f.write(f'TOTAL {len(checks)} / PASS {result["passCount"]} / FAIL {result["failCount"]}\n')
print((out/'v13101_print_verify.log').read_text(encoding='utf-8'))
