...
check('申請番号管理の現行版識別',appsHtml.includes('content="part533"'));
...
check('固縛力画面の現行版識別',ctuHtml.includes('content="part533"'));
...
check('VERSION part533',version.version==='part533'&&version.part===533);
...
const report={generatedAt:new Date().toISOString(),release:'part533',status:failed.length?'failed':'passed',total:checks.length,passed:checks.length-failed.length,failed:failed.length,checks};
const out=path.join(root,'docs','part533_Part529共通案件情報回帰検証レポート.json');
