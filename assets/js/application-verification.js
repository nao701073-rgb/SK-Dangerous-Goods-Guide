...
(function(){
  ...
  document.addEventListener('DOMContentLoaded',()=>{$('saveVerificationResult')?.addEventListener('click',()=>{const msg=$('verificationRegistrationMessage');try{if(!latest)throw new Error('先にExcel申請書を解析してください。');const row=window.ISSApplicationResults.save($('verificationApplicationSelect').value,'dangerous-goods-verification','申請書確認結果',latest);msg.textContent=`申請番号 ${row.applicationYear}-${row.applicationNumber} に登録しました。`;}catch(e){msg.textContent=e.message||'登録できませんでした。';}})});
})();
