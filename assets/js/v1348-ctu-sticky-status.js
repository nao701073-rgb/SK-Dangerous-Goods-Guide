(()=>{
  ...
  const SECURING_FIELDS=new Set([
    'quickMaterialCategory','quickMethod','quickMaterial','quickCount','quickStrength','quickCargoMsl','quickCtuMsl','quickAngle','quickBasis',
    'quickSupportCount','quickSupportStrength','quickSupportBasis','quickCombinationConfirmed','quickDirection','quickSupportDirection',
    'quickFriction','quickMu'
  ]);
  const REQUIRED_PROVENANCE=new Set(['quickMass','quickMaterial','quickCount','quickStrength','quickCargoMsl','quickCtuMsl','quickAngle','quickBasis','quickSupportCount','quickSupportStrength','quickSupportBasis','quickCombinationConfirmed']);
  ...
  function stepTarget(step){const card=stepCard(step);if(card?.id)return card.id;return step===1?'ctuExcelRoutePanel':step===2?'photoInputPanel':step===3?'quickTransport':step===4?'v1ContactStep':step===5?'quickMaterialCategory':'quickCalcBtn'}
  ...
  function evaluate(){
    ...
    const supportOnly=category==='support',combined=category==='combined',tensile=!supportOnly;
    ...
    if(!Number.isFinite(count)||count<=0||!hasAccepted('quickCount'))missing.push(item(supportOnly?'支保材の個数':'固縛本数','quickCount','securing','missing',5));
    const strength=number('quickStrength');
    if(!Number.isFinite(strength)||strength<=0||!hasAccepted('quickStrength'))missing.push(item(supportOnly?'確認済み支保力':'確認済みMSL/STF','quickStrength','securing','missing',5));
    if(!value('quickMaterial')||!hasAccepted('quickMaterial'))missing.push(item('固縛・支保材質','quickMaterial','securing','missing',5));
    if(tensile&&method==='direct'){
    ...
    if(supportOnly&&(!value('quickBasis')||!hasAccepted('quickBasis')))review.push(item('支保力の根拠','quickBasis','securing','review',5));
    const aiLabels={quickMaterial:'固縛・支保材質（AI候補）',quickCount:'固縛本数（AI候補）',quickAngle:'鉛直角（AI候補）',quickSupportMaterial:'支保材質（AI候補）',quickSupportCount:'支保材個数（AI候補）'};
    ...
    if(combined){
      const supportCount=number('quickSupportCount');if(!Number.isFinite(supportCount)||supportCount<=0||!hasAccepted('quickSupportCount'))missing.push(item('併用支保材の個数','quickSupportCount','securing','missing',5));
      const supportStrength=number('quickSupportStrength');if(!Number.isFinite(supportStrength)||supportStrength<=0||!hasAccepted('quickSupportStrength'))missing.push(item('併用支保材の支保力','quickSupportStrength','securing','missing',5));
      if(!value('quickSupportBasis')||!hasAccepted('quickSupportBasis'))review.push(item('併用支保力の根拠','quickSupportBasis','securing','review',5));
      if(!$('quickCombinationConfirmed')?.checked||!hasAccepted('quickCombinationConfirmed'))review.push(item('併用の荷重分担・作用方向','quickCombinationConfirmed','securing','review',5));
    }
  ...
  function confirmAllCurrent(){
    ...
    if(($('quickMaterialCategory')?.value||'')==='combined'){const c=$('quickCombinationConfirmed');if(c){c.checked=true;accept('quickCombinationConfirmed')}}
  ...
  window.__SK_ASSET_BUILD__=Object.assign(window.__SK_ASSET_BUILD__||{}, {'assets/js/v1348-ctu-sticky-status.js':'v1.3.75-confirm-all'});
})();
