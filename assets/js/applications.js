(() => {
  ...
  const fields = {
    ...
    applicantName: $("applicantName"), shipper: $("shipperName"), loadingPort: $("loadingPort"), dischargePort: $("dischargePort"), containerType: $("containerType")
  };
  const cargoEditor = window.ISSApplicationCase?.CargoEditor ? new window.ISSApplicationCase.CargoEditor($("applicationCargoEditor"), { items:[] }) : null;
  ...
  function toggleNumberMode(){ if(!fields.mode)return; const temp=fields.mode.value==="temporary"; const official=$("officialApplicationNumberField"), temporary=$("temporaryApplicationNumberField"); if(official)official.hidden=temp; if(temporary)temporary.hidden=!temp; if(fields.number)fields.number.required=!temp; if(fields.tempNumber)fields.tempNumber.required=temp; }
  ...
  function filteredApplications(){
    ...
    const rows=currentApplications().filter(i=>{
      ...
      const caseData=window.ISSApplicationCase?.extractCaseData?.(i)||{};const cargoText=(caseData.cargoItems||[]).map(row=>[row.unNumber,row.originalName,row.properShippingNameJa,row.properShippingNameEn,row.containerCode,row.packingInstruction].join(" ")).join(" ");
      const h=[i.applicationYear,displayNumber(i),i.containerNumber,i.vesselName,i.voyageNumber,i.unNumber,i.japaneseName,i.englishName,i.caseTitle,i.assignee,i.note,i.office,caseData.applicantName,caseData.shipper,caseData.loadingPort,caseData.dischargePort,caseData.containerType,cargoText].join(" ").toLowerCase();
  ...
  function resetEditMode(){ editingId.value=""; form.reset(); cargoEditor?.setItems([]); fields.year.value=currentYear(); fields.status.value="draft"; fields.mode.value="official"; registrationOffice.value=window.ISSStorage.getOfficeId(); registrationOffice.disabled=!window.ISSStorage.isSafetyEnvironment()||!canWrite(); fields.number.disabled=false; submitButton.textContent="登録"; cancelEditButton.hidden=true; toggleNumberMode(); }
  function beginEdit(id){ const i=currentApplications().find(r=>r.id===id); if(!i)return; const caseData=window.ISSApplicationCase?.extractCaseData?.(i)||{}; editingId.value=i.id; fields.year.value=i.applicationYear||currentYear(); fields.mode.value=i.numberType||"official"; fields.number.value=i.numberType==="temporary"?"":i.applicationNumber||""; fields.tempNumber.value=i.temporaryNumber||""; fields.status.value=statusLabels[i.status]?i.status:"in_progress"; fields.assignee.value=i.assignee||""; fields.containerNumber.value=i.containerNumber||""; fields.vesselName.value=i.vesselName||""; fields.voyageNumber.value=i.voyageNumber||""; fields.unNumber.value=i.unNumber||""; fields.japaneseName.value=i.japaneseName||i.cargoName||""; fields.englishName.value=i.englishName||""; fields.hazardClass.value=i.hazardClass||""; [...fields.subsidiary.options].forEach(o=>o.selected=(i.subsidiaryHazardClasses||[]).includes(o.value)); fields.packingGroup.value=i.packingGroup||""; fields.applicationDate.value=i.applicationDate||""; fields.plannedDate.value=i.inspectionPlannedDate||""; fields.inspectionDate.value=i.inspectionDate||""; fields.caseTitle.value=i.caseTitle||""; fields.note.value=i.note||""; fields.applicantName.value=caseData.applicantName||""; fields.shipper.value=caseData.shipper||i.shipper||""; fields.loadingPort.value=caseData.loadingPort||""; fields.dischargePort.value=caseData.dischargePort||""; fields.containerType.value=caseData.containerType||""; cargoEditor?.setItems(caseData.cargoItems||[]); registrationOffice.value=i.officeId||window.ISSStorage.getOfficeId(); registrationOffice.disabled=true; submitButton.textContent="更新"; cancelEditButton.hidden=false; toggleNumberMode(); form.scrollIntoView({behavior:"smooth",block:"start"}); }
  function cargoDetails(items){
    const rows=window.ISSApplicationCase?.normalizeCargoItems?.(items)||[];
    if(!rows.length)return "";
    const f=value=>Number.isFinite(Number(value))?Number(value).toLocaleString("ja-JP",{maximumFractionDigits:3}):"―";
    const quantity=row=>`<div class="cargo-quantity"><strong>個数 ${f(row.packageCount)}個</strong><strong>1容器当たり正味質量 ${f(row.netMassPerPackageKg)} kg</strong><strong>1容器当たり総質量 ${f(row.grossMassPerPackageKg)} kg</strong><span>申請総正味質量（N/W） ${f(row.totalNetMassKg)} kg</span><span>申請総質量（G/W） ${f(row.totalGrossMassKg)} kg</span></div>`;
    const body=rows.map((row,index)=>{const fallbackSource=window.ISSApplicationCase?.sourceLink?.(row)||"";const [fallbackHref,fallbackLabel]=fallbackSource.split("|");const href=row.referenceSourceHref||fallbackHref||"";const label=row.referenceSourceLabel||fallbackLabel||"";const ja=row.originalName||row.properShippingNameJa||"品名要確認";const en=row.properShippingNameEn||"";return `<tr><td>${index+1}</td><td><span class="cargo-un">${row.unNumber?`UN${escapeHtml(row.unNumber)}`:"国連番号要確認"}</span></td><td><strong>${escapeHtml(ja)}</strong>${en&&en!==ja?`<span class="cargo-original">${escapeHtml(en)}</span>`:""}</td><td>${escapeHtml(row.packingGroup||"―")}</td><td>${escapeHtml(row.containerCode||"―")}</td><td>${quantity(row)}</td><td><div class="cargo-allowance"><strong>許容容量・許容質量</strong>${row.referenceRequirementCode?`<span class="cargo-reference-code">参照要件 ${escapeHtml(row.referenceRequirementCode)}</span>`:""}<p>${escapeHtml(row.allowableCapacityOrMass||"原文・容器表示を確認してください。")}</p>${row.allowanceStatusText?`<small class="cargo-allowance-status">${escapeHtml(row.allowanceStatusText)}</small>`:""}${row.permissionNote?`<div class="cargo-permission"><strong>許可・確認事項</strong><br>${escapeHtml(row.permissionNote)}</div>`:""}${href?`<div class="review-links"><a href="${escapeHtml(href)}" target="_blank" rel="noopener">${escapeHtml(label||"原文を開く")}</a></div>`:""}</div></td></tr>`}).join("");
    return `<details class="application-history application-cargo-details" open><summary>危険物明細 ${rows.length}件を確認</summary><div class="application-cargo-table-wrap" role="region" aria-label="登録済み危険物明細"><table class="application-cargo-table"><thead><tr><th>No.</th><th>国連番号</th><th>品名・原文</th><th>容器等級</th><th>容器コード</th><th>申請数量</th><th>許容容量・許容質量</th></tr></thead><tbody>${body}</tbody></table></div></details>`;
  }
  ...
  function applicationDetailHtml(i){
    if(!i)return '<p>登録申請を確認できません。</p>';
    const c=window.ISSApplicationCase?.extractCaseData?.(i)||{},cargo=c.cargoItems||[],totals=window.ISSApplicationCase?.aggregateCargoItems?.(cargo)||{};
    const d=displayNumber(i);
    const meta=[['申請年度',i.applicationYear||'―'],['申請番号',d||'―'],['申請者・依頼元',c.applicantName||i.applicantName||'―'],['荷主・荷送人',c.shipper||i.shipper||'―'],['コンテナ番号',i.containerNumber||'―'],['コンテナ種類',c.containerType||i.containerType||'―'],['船名',i.vesselName||'―'],['航海番号',i.voyageNumber||'―'],['船積港',c.loadingPort||i.loadingPort||'―'],['陸揚港',c.dischargePort||i.dischargePort||'―']];
    return `<section class="application-detail-summary"><div class="application-detail-number"><span>申請番号</span><strong>${escapeHtml(i.applicationYear||'')}年度・${escapeHtml(d||'―')}</strong></div><div class="application-detail-totals"><span>危険物 ${cargo.length}件</span>${Number(totals.totalNetMassKg)?`<span>申請総N/W ${Number(totals.totalNetMassKg).toLocaleString('ja-JP',{maximumFractionDigits:3})} kg</span>`:''}${Number(totals.totalGrossMassKg)?`<span>申請総G/W ${Number(totals.totalGrossMassKg).toLocaleString('ja-JP',{maximumFractionDigits:3})} kg</span>`:''}</div></section><dl class="application-detail-meta">${meta.map(([k,v])=>`<div><dt>${escapeHtml(k)}</dt><dd>${escapeHtml(v)}</dd></div>`).join('')}</dl>${cargoDetails(cargo)}${i.note?`<section class="application-detail-note"><h3>備考</h3><p>${escapeHtml(i.note)}</p></section>`:''}`;
  }
  function openApplicationDetail(id){
    if(!id)return false;
    const url=`application-detail.html?applicationId=${encodeURIComponent(id)}`;
    const win=window.open(url,'_blank','noopener');
    if(!win)location.href=url;
    return true;
  }
  function closeApplicationDetail(){ return true; }
  function render(){ const apps=filteredApplications(); renderSummary(); renderActiveFilters(apps.length); if(!apps.length){list.innerHTML='<div class="empty-state"><strong>条件に一致する申請案件がありません</strong><p>検索語、申請年度、進捗状況または表示範囲を確認してください。</p><button id="clearEmptyApplicationFilters" type="button">絞り込み条件を解除</button></div>';$('clearEmptyApplicationFilters').onclick=()=>{filter.value="";statusFilter.value="";yearFilter.value="";if(window.ISSStorage.isSafetyEnvironment()&&scopeSelect.value==="all")officeFilter.value="";render();};return;}
    ...
    list.innerHTML=apps.map(i=>{ const pc=photos.filter(p=>p.applicationId===i.id).length; const dc=documents.filter(d=>d.applicationId===i.id).length; const caseData=window.ISSApplicationCase?.extractCaseData?.(i)||{};const cargo=caseData.cargoItems||[];const totals=window.ISSApplicationCase?.aggregateCargoItems?.(cargo)||{}; const name=i.caseTitle||i.japaneseName||i.englishName||(cargo[0]?.originalName)||""; const meta=[display.container&&i.containerNumber&&`コンテナ番号：${i.containerNumber}`,caseData.containerType&&`コンテナ種類：${caseData.containerType}`,display.vessel&&i.vesselName&&`船名：${i.vesselName}`,display.vessel&&i.voyageNumber&&`航海番号：${i.voyageNumber}`,caseData.loadingPort&&`船積港：${caseData.loadingPort}`,caseData.dischargePort&&`陸揚港：${caseData.dischargePort}`,cargo.length&&`危険物：${cargo.length}件`,totals.totalGrossMassKg&&`申請総質量：${Number(totals.totalGrossMassKg).toLocaleString("ja-JP",{maximumFractionDigits:3})} kg`].filter(Boolean);
      ...
      return `<article class="application-card application-card--compact"><div class="application-card__header"><div><span class="application-number">${escapeHtml(i.applicationYear||"")}年度・${escapeHtml(displayNumber(i))}</span>${name?`<h3>${escapeHtml(name)}</h3>`:""}<span class="application-card__office">${escapeHtml(i.blockName)}｜${escapeHtml(i.office)}</span></div><span class="record-status" data-status="${escapeHtml(i.status||"draft")}">${escapeHtml(statusLabels[i.status]||i.status)}</span></div><div class="application-compact-facts">${i.containerNumber?`<span>コンテナ ${escapeHtml(i.containerNumber)}</span>`:""}${i.vesselName?`<span>船名 ${escapeHtml(i.vesselName)}</span>`:""}<span>危険物 ${cargo.length}件</span><span>写真 ${pc}枚</span><span>資料 ${dc}件</span><span>更新 ${escapeHtml(formatDate(i.updatedAt||i.createdAt))}</span></div><div class="management-actions application-detail-action"><a class="primary-action application-detail-newtab" href="application-detail.html?applicationId=${encodeURIComponent(i.id)}" target="_blank" rel="noopener">詳細を新しいタブで開く</a>${canWrite()?`<a class="button-link" href="ctu-securing-calculator.html?applicationId=${encodeURIComponent(i.id)}">固縛力参考算出</a><button data-edit-application="${escapeHtml(i.id)}" type="button">編集</button>`:''}</div></article>`; }).join("");
    document.querySelectorAll("[data-view-application-detail]").forEach(b=>b.onclick=()=>openApplicationDetail(b.dataset.viewApplicationDetail));
  ...
  function exportCsv(){const rows=filteredApplications();if(!rows.length)return alert("出力できる申請番号がありません。"); const table=[["申請年度","正式申請番号","仮番号","事業所","担当者","進捗状況","申請者・依頼元","荷主・荷送人","コンテナ番号","コンテナ種類","船名","航海番号","船積港","陸揚港","危険物件数","国連番号","品名・原文","容器等級","容器コード","個数","1容器N/W(kg)","1容器G/W(kg)","総N/W(kg)","総G/W(kg)","包装要件・法令参照","許容容量・許容質量","許可確認","申請日","検査予定日","検査実施日","備考","更新日時"]]; rows.forEach(i=>{const c=window.ISSApplicationCase?.extractCaseData?.(i)||{};const goods=c.cargoItems?.length?c.cargoItems:[{}];goods.forEach((g,index)=>table.push([i.applicationYear,index?"":i.numberType==="temporary"?"":i.applicationNumber,index?"":i.temporaryNumber,index?"":i.office,index?"":i.assignee,index?"":statusLabels[i.status]||i.status,index?"":c.applicantName,index?"":c.shipper,index?"":i.containerNumber,index?"":c.containerType,index?"":i.vesselName,index?"":i.voyageNumber,index?"":c.loadingPort,index?"":c.dischargePort,index?"":goods.length,g.unNumber?`UN${g.unNumber}`:"",g.originalName||g.properShippingNameJa||g.properShippingNameEn||"",g.packingGroup,g.containerCode,g.packageCount,g.netMassPerPackageKg,g.grossMassPerPackageKg,g.totalNetMassKg,g.totalGrossMassKg,g.packingInstruction,g.allowableCapacityOrMass,g.permissionRequired?"必要":"",index?"":i.applicationDate,index?"":i.inspectionPlannedDate,index?"":i.inspectionDate,index?"":i.note,index?"":i.updatedAt]));}); const blob=new Blob(["\ufeff"+table.map(r=>r.map(csvEscape).join(",")).join("\r\n")],{type:"text/csv;charset=utf-8"}); const url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`申請番号一覧_${new Date().toISOString().slice(0,10)}.csv`;a.click();URL.revokeObjectURL(url);}
  function payload(){const cargoItems=cargoEditor?.getItems?.()||[];const compatibility=window.ISSApplicationCase?.compatibilityFromCargo?.(cargoItems)||{};const caseData={applicantName:fields.applicantName.value,shipper:fields.shipper.value,loadingPort:fields.loadingPort.value,dischargePort:fields.dischargePort.value,containerType:fields.containerType.value,cargoItems};return {applicationYear:fields.year.value,numberType:fields.mode.value,applicationNumber:fields.number.value,status:fields.status.value,assignee:fields.assignee.value,containerNumber:fields.containerNumber.value,vesselName:fields.vesselName.value,voyageNumber:fields.voyageNumber.value,...compatibility,applicantName:fields.applicantName.value,shipper:fields.shipper.value,loadingPort:fields.loadingPort.value,dischargePort:fields.dischargePort.value,containerType:fields.containerType.value,cargoItems,caseData,applicationDate:fields.applicationDate.value,inspectionPlannedDate:fields.plannedDate.value,inspectionDate:fields.inspectionDate.value,caseTitle:fields.caseTitle.value,note:fields.note.value,officeId:registrationOffice.value};}
  ...
  populateOrganizationControls();applyDisplaySettingsToControls();renderSavedViews();resetEditMode();const requestedParams=new URLSearchParams(location.search),requestedStatus=requestedParams.get("status"),requestedQuery=requestedParams.get("query"),requestedSort=requestedParams.get("sort"),requestedApplicationId=requestedParams.get("applicationId");if(requestedStatus&&statusLabels[requestedStatus])statusFilter.value=requestedStatus;if(requestedQuery)filter.value=requestedQuery;if(requestedSort&&[...sortSelect.options].some(option=>option.value===requestedSort))sortSelect.value=requestedSort;render();$("applicationDetailClose")?.addEventListener("click",closeApplicationDetail);$("applicationDetailCloseBottom")?.addEventListener("click",closeApplicationDetail);$("applicationDetailDialog")?.addEventListener("click",e=>{if(e.target===$("applicationDetailDialog"))closeApplicationDetail();});if(requestedApplicationId){const app=currentApplications().find(row=>String(row.id)===String(requestedApplicationId));if(app){filter.value=displayNumber(app)||"";render();setTimeout(()=>openApplicationDetail(requestedApplicationId),0);}}
})();

window.__SK_ASSET_BUILD__ = Object.assign(window.__SK_ASSET_BUILD__ || {}, { "assets/js/applications.js": "part549" });
