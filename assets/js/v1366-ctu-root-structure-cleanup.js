...
function removeKnownLegacyUi(){
  ...
  if($('ctuStep5Panels')){
    ...
    if(legacy){legacy.hidden=true;legacy.setAttribute('aria-hidden','true');legacy.dataset.v1366LegacyEmpty='1';}
  }
}
function removeEmptyLegacySiblings(){
  ...
  anchors.forEach(anchor=>{
    ...
    for(let i=0;i<3&&node;i++){
      ...
      if(node.matches?.(known)&&!hasUseful(node)){node.dataset.v1366LegacyEmpty='1';node.remove();}
      node=prev;
    }
...
window.__SK_ASSET_BUILD__=Object.assign(window.__SK_ASSET_BUILD__||{}, {'assets/js/v1366-ctu-root-structure-cleanup.js':'v1.3.66'});
})();
