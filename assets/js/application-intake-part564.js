(function(){
  ...
  const ui={showReviewOnly:false,issueCursor:-1,scheduled:false,lastStage:'',reviewed:new Set(),reviewerCandidateApplied:false,reviewBoardState:'active',reviewReason:'all',sessionLoadedFor:''};
  ...
  function ensureReviewBoard(){
    ...
    board.innerHTML='<summary><span><strong>確認対象一覧</strong><small>未確認・要修正をまとめて確認</small></span><b id="part563ReviewBoardCount">0件</b></summary><div class="part563-review-board__body"><p class="part563-review-board__lead">入力修正が必要な項目と、人による原文・許可・条件確認が必要な項目を分けて表示します。各項目から該当する危険物へ直接移動できます。</p><div class="part564-review-filters"><div class="part564-review-state-filters" id="part564ReviewStateFilters"><button type="button" data-part564-state="active" class="is-active">未確認・要修正</button><button type="button" data-part564-state="hard">要修正</button><button type="button" data-part564-state="pending">人確認待ち</button><button type="button" data-part564-state="ack">確認済み</button><button type="button" data-part564-state="all">すべて</button></div><div class="part564-review-reason-filters" id="part564ReviewReasonFilters"></div></div><div class="part564-review-counts" id="part564ReviewCounts"></div><div id="part563ReviewList"></div></div>';
  ...
  function renderReviewBoard(ctx){
    ...
    const allTargets=ctx.infos.filter(i=>i.systemIssue);const global=ctx.globalIssues.filter(t=>!/危険物No\.\d+/.test(t));
    const hard=allTargets.filter(i=>i.hard).length,pending=allTargets.filter(i=>!i.hard&&!i.acknowledged).length,ack=allTargets.filter(i=>i.acknowledged).length;
    count.textContent=`${hard+pending+global.length}件`;
    const counts=$('part564ReviewCounts');if(counts)counts.innerHTML=`<span class="is-hard">要修正 <b>${hard}</b></span><span class="is-pending">人確認待ち <b>${pending}</b></span><span class="is-ack">確認済み <b>${ack}</b></span>`;
    qa('[data-part564-state]',board).forEach(b=>b.classList.toggle('is-active',b.dataset.part564State===ui.reviewBoardState));
    const reasonNode=$('part564ReviewReasonFilters');if(reasonNode){const reasons=[...new Set(allTargets.flatMap(i=>i.reasons))];reasonNode.innerHTML=`<button type="button" data-part564-reason="all" class="${ui.reviewReason==='all'?'is-active':''}">理由：すべて</button>`+reasons.map(r=>`<button type="button" data-part564-reason="${esc(r)}" class="${ui.reviewReason===r?'is-active':''}">${esc(r)}</button>`).join('');}
    const rowTargets=filteredReviewRows(ctx);
    if(!allTargets.length&&!global.length){list.innerHTML='<div class="part563-review-empty"><strong>✓ 確認対象はありません</strong><span>危険物の自動確認と人による確認が完了しています。</span></div>';board.open=false;return}
    if((hard+pending)>0&&!board.dataset.part564Opened){board.open=true;board.dataset.part564Opened='true'}
    ...
    const globals=global.length&&ui.reviewBoardState!=='ack'?`<div class="part563-global-issues"><strong>申請全体の確認事項</strong>${global.map(t=>`<p>${esc(t)}</p>`).join('')}</div>`:'';
    const empty=!rows?'<div class="part564-filter-empty">この条件に該当する危険物はありません。</div>':'';list.innerHTML=globals+rows+empty;
  }
  ...
  function reviewSessionId(){
    const app=applicationLabel();const applicant=String($('intakeApplicant')?.value||'').trim();const cargo=rowsInfo().map(i=>i.key).join('||');
    return `part564:${app}:${applicant}:${cargo}`;
  }
  function loadReviewSession(){
    const id=reviewSessionId();if(!id||id===ui.sessionLoadedFor)return;ui.sessionLoadedFor=id;
    try{const raw=sessionStorage.getItem('skdg:application-review:'+id);if(!raw)return;const saved=JSON.parse(raw);if(Array.isArray(saved?.reviewed)){const valid=new Set(rowsInfo().map(i=>i.key));ui.reviewed=new Set(saved.reviewed.filter(k=>valid.has(k)));}}
    catch{}
  }
  function saveReviewSession(){
    const id=reviewSessionId();if(!id)return;try{const valid=new Set(rowsInfo().map(i=>i.key));const reviewed=[...ui.reviewed].filter(k=>valid.has(k));sessionStorage.setItem('skdg:application-review:'+id,JSON.stringify({reviewed,savedAt:new Date().toISOString()}));}catch{}
  }
  function clearReviewSession(){const id=reviewSessionId();try{if(id)sessionStorage.removeItem('skdg:application-review:'+id)}catch{}ui.sessionLoadedFor='';}
  function filteredReviewRows(ctx){
    return ctx.infos.filter(i=>{
      const stateOk=ui.reviewBoardState==='all'||(ui.reviewBoardState==='active'&&i.issue)||(ui.reviewBoardState==='hard'&&i.hard)||(ui.reviewBoardState==='pending'&&i.systemIssue&&!i.hard&&!i.acknowledged)||(ui.reviewBoardState==='ack'&&i.acknowledged);
      const reasonOk=ui.reviewReason==='all'||i.reasons.includes(ui.reviewReason);return stateOk&&reasonOk;
    });
  }
  ...
  function render(){
    ui.scheduled=false;ensureGuide();let ctx=collect();loadReviewSession();ctx=collect();applyReviewerCandidate(ctx.checked);ctx=collect();markRows(ctx.infos);const stage=currentStage(ctx),action=actionModel(ctx);ui.lastStage=action.kind;
  ...
  document.addEventListener('click',e=>{
    ...
    if(row&&e.target.closest('[data-part562-ack]')){const i=rowInfo(row);if(i.systemIssue&&!i.hard){ui.reviewed.add(i.key);saveReviewSession();ui.issueCursor=-1;render();setTimeout(()=>{if(collect().issues)nextIssue()},120)}return}
    if(row&&e.target.closest('[data-part562-unack]')){const i=rowInfo(row);ui.reviewed.delete(i.key);saveReviewSession();ui.issueCursor=-1;render();return}
    if(row&&e.target.closest('[data-part562-fix]')){focusFixField(rowInfo(row));return}
    const stateFilter=e.target.closest('[data-part564-state]');if(stateFilter){ui.reviewBoardState=stateFilter.dataset.part564State||'active';render();return}
    const reasonFilter=e.target.closest('[data-part564-reason]');if(reasonFilter){ui.reviewReason=reasonFilter.dataset.part564Reason||'all';render();return}
    const boardIndexAttr=e.target.closest('[data-part563-jump],[data-part563-fix],[data-part563-ack],[data-part563-unack]');if(boardIndexAttr){const idx=Number(boardIndexAttr.dataset.part563Jump??boardIndexAttr.dataset.part563Fix??boardIndexAttr.dataset.part563Ack??boardIndexAttr.dataset.part563Unack);const info=rowsInfo().find(i=>i.index===idx);if(!info)return;if(boardIndexAttr.hasAttribute('data-part563-jump')){jumpToInfo(info);return}if(boardIndexAttr.hasAttribute('data-part563-fix')){focusFixField(info);return}if(boardIndexAttr.hasAttribute('data-part563-ack')&&info.systemIssue&&!info.hard){ui.reviewed.add(info.key);saveReviewSession();ui.issueCursor=-1;render();return}if(boardIndexAttr.hasAttribute('data-part563-unack')){ui.reviewed.delete(info.key);saveReviewSession();ui.issueCursor=-1;render();return}}
  });
  ...
  document.addEventListener('click',e=>{if(e.target.closest('#intakeClear'))clearReviewSession();if(e.target.closest('#intakeRunCheck,#intakeRegisterApplication,#intakeClear,#intakeAddCargo,[data-remove-cargo],#intakeOpenApplication'))setTimeout(schedule,100)});
  ...
  ensureGuide();render();
  window.__SK_ASSET_BUILD__=Object.assign(window.__SK_ASSET_BUILD__||{}, {'assets/js/application-intake-part564.js':'part564'});
})();
