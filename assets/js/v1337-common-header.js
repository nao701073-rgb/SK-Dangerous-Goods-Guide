...
function updateUser(){
 ...
 document.querySelectorAll('.sk-v1337-header .sk-current-user-link').forEach(a=>{
...
function createDrawer(){
 let d=document.querySelector('.sk-v1337-drawer');if(d)return d;
 d=document.createElement('aside');d.className='sk-v1337-drawer';d.setAttribute('aria-hidden','true');
 d.innerHTML='<div class="sk-v1337-drawer__head"><strong>メニュー</strong><button type="button" class="sk-v1337-drawer__close" aria-label="メニューを閉じる">'+CLOSE+'</button></div><div class="sk-v1337-drawer__section">メインメニュー</div><nav class="sk-v1337-drawer__nav"></nav>';
 const nav=d.querySelector('nav');
 for(const [ic,label,href] of navItems()){
   const a=document.createElement('a');a.href=href;if(activeFor(href))a.setAttribute('aria-current','page');a.innerHTML='<span class="sk-v1337-drawer__icon">'+(ICONS[ic]||HOME)+'</span><span></span>';a.lastElementChild.textContent=label;nav.appendChild(a);
 }
 d.querySelector('.sk-v1337-drawer__close').addEventListener('click',closeMenu);document.body.appendChild(d);return d;
}
function openMenu(){createDrawer();if(!document.querySelector('.sk-v1337-backdrop')){const b=document.createElement('div');b.className='sk-v1337-backdrop';b.addEventListener('click',closeMenu);document.body.appendChild(b)}document.documentElement.classList.add('sk-v1337-menu-open');document.querySelector('.sk-v1337-drawer')?.setAttribute('aria-hidden','false');document.querySelector('.sk-v1337-menu-button')?.setAttribute('aria-expanded','true')}
function closeMenu(){document.documentElement.classList.remove('sk-v1337-menu-open');document.querySelector('.sk-v1337-drawer')?.setAttribute('aria-hidden','true');document.querySelector('.sk-v1337-backdrop')?.remove();document.querySelector('.sk-v1337-menu-button')?.setAttribute('aria-expanded','false')}
function init(){
 document.documentElement.classList.remove('sk-v1337-menu-open');document.body.classList.add('has-sk-v1337-header');
 updateUser();requestAnimationFrame(updateUser);setTimeout(updateUser,400);document.querySelector('.sk-v1337-menu-button')?.addEventListener('click',openMenu,{passive:true});
